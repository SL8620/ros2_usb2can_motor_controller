...
private:
    ...
    void init_communications() {
        ...
        status_sub_ = create_subscription<motor_interfaces::msg::MotorStatus>(
            "/motor_status", qos,
            [this](motor_interfaces::msg::MotorStatus::ConstSharedPtr msg) {
        ...
        stats_timer_ = create_wall_timer(
            ...
            [this]() { 
                ...
                // 检查是否超时
                if (elapsed_time() >= test_duration_) {
    ...
    void start_test() {
        ...
        RCLCPP_INFO(get_logger(), "Test started. Duration: %.1f seconds", test_duration_);
    }
    ...
    void send_disable_commands() {
        ...
        for (const auto& motor : motors_) {
            ...
            cmd_msg.channel = motor.channel;
            cmd_msg.id = motor.id;
            cmd_msg.command_type = 1; // 禁用命令
            command_pub_->publish(cmd_msg);
        }
        RCLCPP_INFO(get_logger(), "Sent disable commands to all motors");
    }
    ...
    void print_statistics() 
    {
        ...
        RCLCPP_INFO(get_logger(), "\n===== Real-time Statistics (Elapsed: %.1f/%.1f sec) =====", 
                   elapsed, test_duration_);
        RCLCPP_INFO(get_logger(), "Publish Count: %zu (%.1f Hz)", 
                publish_count_.load(), publish_count_.load() / elapsed);
        ...
        for (const auto& motor : motors_) 
        {
            ...
            RCLCPP_INFO(get_logger(), 
                ...
                motor.name.c_str(), id, 
                current_freq_[id].load(),  // 显式加载原子值
                msg_counts_[id].load());   // 显式加载原子值
        }
    }
    ...
    void print_final_statistics() 
    {
        ...
        RCLCPP_INFO(get_logger(), "Actual Publish Rate: %.3f Hz", 
                publish_count_.load() / total_time);

        for (const auto& motor : motors_) 
        {
            const uint8_t id = motor.id;
            const double avg_freq = msg_counts_[id].load() / total_time;
            RCLCPP_INFO(get_logger(), 
                "[%s] ID: %-2d | Avg Freq: %6.1f Hz | Total Received: %zu",
                motor.name.c_str(), id, avg_freq, msg_counts_[id].load());
        }
    }
