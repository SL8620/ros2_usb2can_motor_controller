...
public:
    ...
    MotorNode() : Node("motor_node") {
        ...
        // 在motor_node.cpp中添加（构造函数内）：
        auto qos = rclcpp::QoS(rclcpp::KeepLast(24))
        ...
        command_sub_ = create_subscription<MotorCommand>(
            "/motor_command",
            qos,
            std::bind(&MotorNode::on_command, this, _1));  // 直接绑定
        ...
        control_sub_ = create_subscription<MotorControl>(
            ...
            std::bind(&MotorNode::on_control, this, _1));
                ...
                status_pub_ = create_publisher<MotorStatus>(
                    "/motor_status",
                    qos  // 发布端同样配置
                );
            }
...
private:
    ...
    void on_control(const MotorControl::SharedPtr msg) 
    {
        ...
        // 拆分检查发送流程
        // 修改后（正确）
        parser->packCommand(*msg, can_msg.data);  // 直接调用，不捕获返回值
        bool prep_result = true;  // 假设打包总是成功

        bool send_result = dev_it->second->sendCanMessage(can_msg);
      std::stringstream ss;
        ss << "CAN Msg - ID: 0x" << std::hex << std::setw(8) << std::setfill('0') << can_msg.id
        << " Len: " << std::dec << can_msg.data.size()
        << " Data: ";

        for (uint8_t byte : can_msg.data) {
        ...
        if (send_result) 
        {
            RCLCPP_INFO(get_logger(), "Successfully sent to %s", key.c_str());
        } 
        else 
        {
            RCLCPP_ERROR(get_logger(), "Failed to send to %s", key.c_str());  // 提升为ERROR级别
        }
