  ...
  motor_interfaces__msg__MotorStatus__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    motor_interfaces__msg__MotorStatus * data =
      (motor_interfaces__msg__MotorStatus *)allocator.reallocate(
    ...
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!motor_interfaces__msg__MotorStatus__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          motor_interfaces__msg__MotorStatus__fini(&output->data[i]);
        }
        return false;
      }
    }
