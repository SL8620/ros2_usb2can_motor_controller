  ...
  motor_interfaces__msg__MotorControl__Sequence * output)
{
  ...
  if (output->capacity < input->size) {
    ...
    motor_interfaces__msg__MotorControl * data =
      (motor_interfaces__msg__MotorControl *)realloc(output->data, allocation_size);
    if (!data) {
      return false;
    }
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!motor_interfaces__msg__MotorControl__init(&data[i])) {
        /* free currently allocated and return false */
        for (; i-- > output->capacity; ) {
          motor_interfaces__msg__MotorControl__fini(&data[i]);
        }
        free(data);
        return false;
      }
